...
from table_test_db import load_entries, save_entry
...
if not get_user_token(headers):
    st.warning(
        "Ingen bruger-token. Aktivér on-behalf-of user authorization og tilføj `sql`-scope "
        "på appen, så tabellen kan læses/skrives med brugerens rettigheder."
    )
    st.stop()
